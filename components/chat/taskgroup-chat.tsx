...
import { send_message_to_taskgroup_chat, mark_taskgroup_chat_as_read, get_taskgroup_chat_messages } from "@/actions/chat";
import { Message } from "@/types/socket";
import { motion, AnimatePresence } from "framer-motion";
import { MessageBubble, TypingIndicator, LoadingSpinner, EmptyState } from "./chat-components";

interface TaskGroupChatProps {
  taskGroupId: string;
...
export function TaskGroupChat({
  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      const result = await get_taskgroup_chat_messages(taskGroupId);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const result = await send_message_to_taskgroup_chat(taskGroupId, messageContent);
      if (result.message) {
  ...
  const handleMarkAsRead = async () => {
    await mark_taskgroup_chat_as_read(taskGroupId);
    setUnreadCount(0);
  };
    ...
    return (
      ...
      >
        <Button
          ...
          className="relative rounded-full w-16 h-16 shadow-2xl hover:shadow-3xl transition-all duration-300 bg-gradient-to-br from-green-500 via-green-600 to-emerald-700 hover:from-green-600 hover:via-green-700 hover:to-emerald-800 border-2 border-white/20 backdrop-blur-sm"
          size="icon"
        >
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Modern Header */}
        <div className="flex items-center justify-between p-4 bg-gradient-to-r from-green-500 via-green-600 to-emerald-600 text-white relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="font-semibold text-sm truncate">Chat del Grupo</h3>
          ...
          {!isMinimized && (
            ...
            >
              {isLoading ? (
                ...
                <>
                  {/* Messages Area */}
                  <div className="overflow-y-auto p-4 space-y-4 bg-gradient-to-br from-gray-50 via-green-50/30 to-emerald-50/30 dark:from-gray-800 dark:via-green-900/20 dark:to-emerald-900/20" style={{ 
                    ...
                    <form onSubmit={handleSendMessage} className="flex gap-3 items-end">
                      <div className="flex-1 relative">
                        <input
                          ...
                          maxLength={500}
                          className="w-full px-4 py-3 pr-12 bg-gray-100 dark:bg-gray-800 border-0 rounded-2xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white dark:focus:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-200"
