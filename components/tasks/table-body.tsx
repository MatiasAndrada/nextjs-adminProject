...
interface Props {
  tasks: Pick<
    Task,
    "id" | "task_group_id" | "name" | "status" | "progress" | "updatedAt"
  >[];
}
...
export default function TableBody({ tasks }: Props) {
  ...
  return (
    <tbody className="text-md divide-y divide-gray-100 dark:divide-gray-500">
      {tasks.map(({ id, task_group_id, name, status, progress, updatedAt }) => (
        <tr
          ...
          className="
                        ...
                        hover:bg-slate-200 focus:bg-slate-100 hover:dark:bg-slate-800 focus:dark:bg-slate-700 dark:border-gray-700  cursor-pointer
                        "
          onClick={() => {
        ...
        >
          <td className="p-2 whitespace-nowrap">
            <div className="flex items-center gap-2">
              ...
              <div className="font-bold">#{lastDigitsId(id)}</div>
            </div>
          </td>
